...
| `src/shared/agent-log-path.ts` | Computes versioned session log paths from taxonomy |
...
| `AI_MEMORY_DIR` | *(none)* | Path to private memory repo for session logs. Logs go to `$AI_MEMORY_DIR/<repo>/`. Falls back to `<cwd>/session-logs/` if not set. |
| `SESSION_LOG_SPEC_PATH` | *(none)* | Path to `SESSION_LOG_SPEC.md`. If set, its content is injected into the dismiss prompt so the agent writes spec-compliant logs. |
...
`dismiss_daemon` (soft, default) writes a `SESSION_LOG_SPEC`-compliant session log, removes the session from the in-memory registry, but leaves both directories on disk. If the agent fails to write the log (timeout, quota), a fallback stub is generated by the server. The log is git-committed to the memory repo if it's a git working tree.

`dismiss_daemon({ hard: true })` writes the session log, then deletes both directories permanently.
