...
def _now_iso() -> str:
    return _now_local().strftime('%Y-%m-%dT%H:%M:%S')
...
def main():
    ...
    # Source .env for AI_MEMORY_DIR (check both .triumvirate and .claude)
    for env_path in [Path.home() / '.triumvirate' / '.env', Path.home() / '.claude' / '.env']:
        if env_path.exists():
            try:
