...
2. **Prefect flows** on homebox Docker:
   ...
   ssh mboscia@192.168.2.110 'docker exec scripting-host-prefect-worker-1 ls -la /app/flows/'

   # Then trigger the one you need:
   ssh mboscia@192.168.2.110 'docker exec scripting-host-prefect-worker-1 python3 -c "from flows.X import Y; Y(args)"'
   ```
...
ssh mboscia@192.168.2.110 "cd ~/tellus/infrastructure/municipal-bloodhound && docker-compose up -d"

# Or run a specific discovery script
ssh mboscia@192.168.2.110 "docker exec scripting-host-prefect-browser-worker-1 \
  python3 /app/src/discover_water_sewer_gis.py"

# Pull results back
rsync -av mboscia@192.168.2.110:/home/mboscia/zoning_discovery/*.json /tmp/results/
```
...
7. **Municipal Bloodhound on homebox** — for complex multi-page navigation requiring a real browser. This IS browser automation — it runs Playwright + Gemini Flash inside Docker on 192.168.2.110. The browser runs THERE, not here. Your context window only sees the output JSON.
...
### Key Infrastructure (Homebox 192.168.2.110)

| System | What It Does | How to Trigger |
|--------|-------------|----------------|
| Municipal Bloodhound | Playwright + Gemini Flash in Docker — navigates GIS portals, extracts data | `ssh mboscia@192.168.2.110 "cd ~/tellus/infrastructure/municipal-bloodhound && docker-compose up -d"` |
| Prefect Worker | Battle-tested data pipelines (CrUX, refresh flows, ETL) | `ssh mboscia@192.168.2.110 'docker exec scripting-host-prefect-worker-1 python3 -c "..."'` |
| Browser Worker | Headless Playwright for ad-hoc browser tasks | `ssh mboscia@192.168.2.110 "docker exec scripting-host-prefect-browser-worker-1 python3 /app/src/..."` |
