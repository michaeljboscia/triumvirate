...
**Status:** Draft — pending /goatrodeo
...
### REQ-A2.2: Claude Code PreToolUse Hook — File Scope Guard
...
The hook reads `contract.json` from the working directory, inspects the tool input's `file_path`, and returns `exit 2` (deny) if the file is not in `allowed_files`. The deny decision is non-bypassable — even `--dangerously-skip-permissions` cannot override it.
...
### REQ-A2.3: Claude Code PreToolUse Hook — Command Guard

Same pattern for Bash tool calls. Reads `contract.json`, checks against `allowed_commands` and `forbidden_commands`. Blocks dangerous operations (rm -rf, cargo add, dependency modifications) unless explicitly allowed for this task.
