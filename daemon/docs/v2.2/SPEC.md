...
- **REQ-010:** Claude Code hooks (SessionStart, PostToolUse, Stop, SessionEnd) MUST write raw events to the project's spool directory (`<project>/.triumvirate/spool/`) as their PRIMARY action. The hook writes the event JSON to a temp file (`event-<ts>-<pid>-<rand>.tmp`), then atomically renames it to `.ndjson`. Filename generation MUST use POSIX-safe builtins only: `event-$(date +%s)-$$-$RANDOM.tmp`. No dependency on `uuidgen` or external binaries. This eliminates all file contention — no `flock`, no interleaving, no corruption from concurrent hooks. After writing, the hook MUST fire a non-blocking background HTTP ping to the daemon (`POST /ledger/wake` with JSON body `{"project_root": "<absolute path>"}`, no response wait). The hook's ONLY job is: validate payload, atomic spool write, ping daemon, return.
