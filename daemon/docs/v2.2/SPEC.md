...
- **REQ-010:** Claude Code hooks (SessionStart, PostToolUse, Stop, SessionEnd) MUST write raw events to the project's spool directory (`<project>/.triumvirate/spool/`) as their PRIMARY action. The hook writes the event JSON to a temp file (`event-<uuid>.tmp`), then atomically renames it to `event-<uuid>.ndjson`. This eliminates all file contention — no `flock`, no interleaving, no corruption from concurrent hooks. After writing, the hook MUST fire a non-blocking background HTTP ping to the daemon (`POST /ledger/wake` with JSON body `{"project_root": "<absolute path>"}`, no response wait). The hook's ONLY job is: validate payload, atomic spool write, ping daemon, return.
...
- **REQ-010c:** Individual spool event files MUST NOT exceed 64KB. Events exceeding this limit MUST be truncated with a `truncated: true` flag preserved in the payload.
...
- **REQ-037:** Each fleet member agent MUST receive task assignments via two channels:
  - A machine-readable file at `.triumvirate/fleet-task.md` in the worktree, containing frontmatter (task_id, fleet_id, assigned_agent, depends_on) and prose description. The fleet engine MUST commit this file as an initialization commit before spawning the agent, ensuring the agent wakes up to a clean `git status`.
