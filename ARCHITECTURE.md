...
When a daemon is dismissed, it automatically writes a `SESSION_LOG_SPEC`-compliant session log to the private memory repo (`$AI_MEMORY_DIR/<repo>/` or `~/.ai-memory/<repo>/`). If the agent's write fails, a fallback stub log is generated by the MCP server.
...
- `dismiss_daemon` — clean up + automatic session log writing
...
- **Agent log path** (`agent-log-path.ts`) — computes versioned session log paths, reads `taxonomy.json`, resolves `AI_MEMORY_DIR`
...
Session logs are AI working memory — not project artifacts. They live in a dedicated private repo, separate from source code:

```
~/.ai-memory/my-project/                    # or $AI_MEMORY_DIR/my-project/
...
Set `AI_MEMORY_DIR` to point to your private memory repo. Falls back to `<cwd>/session-logs/` if not configured.

Logs are written:
- **Automatically** — by `dismiss_daemon` (writes `SESSION_LOG_SPEC`-compliant log before cleanup)
