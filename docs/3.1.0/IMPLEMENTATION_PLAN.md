...
<task id="T-001" req="REQ-B1,REQ-B2,REQ-B3" wave="0" depends="">
  ...
  <scope_out>Move ONLY the DaemonMetrics struct definition and impl block to daemon-core. Do NOT move the metric registration/initialization logic (prometheus Registry setup, Registry::register calls) — that stays in main.rs and keeps calling into the moved struct. Do NOT move the metrics recording call sites (e.g., state.metrics.agent_requests_total.inc() usages inside handler functions) — those still reference DaemonMetrics through the bus. Do NOT add new metrics. Do NOT touch dashboard, HTTP routes, or MCP handlers.</scope_out>
