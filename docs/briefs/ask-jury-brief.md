...
- Verdict extraction reads a reply, and a reply is prose. **Ambiguity is not a verdict:** every candidate the reader finds is normalized, and they must agree or the seat casts nothing, visibly, with `verdict_note` saying why. Picking one of several was tried both ways and both were wrong. First match votes for an echoed prompt. Last match votes for a word in the seat's own reasoning, which is not hypothetical: on the first live run grok answered `YES\n17 is a prime number because it has no positive divisors`, and last-match recorded `no`. Give a pattern that matches exactly one thing, such as `(?m)^\s*(YES|NO)\s*$`. Nothing defends against a seat whose first line contradicts its own conclusion under the `first_line` default.
- **A line-anchored verdict pattern is fragile with grok.** Grok narrates before it answers, and its narration can run straight into the answer with no separator: three live runs produced `...without asking questions or using Pythia.READY`, so `^\s*(READY|BLOCKED)\s*$` matched nothing and a cast vote was not counted. The adapter is faithful here (it concatenates text deltas, and inserting separators would invent structure grok did not send), so this is the agent's output shape, not a bug to fix. When the verdicts matter, ask for JSON and use `verdict_json_pointer`, which does not depend on where the line breaks fall.
