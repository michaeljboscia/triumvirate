...
1. **`api_state_returns_full_snapshot_with_version_and_uptime`** — populate state with 1 session + 2 abe_tasks + 1 fleet build. GET `/api/state` with bearer. Assert parsed `StateResponse.version == "3.3.0"` (or whatever `daemon_core::VERSION` is — DO NOT hardcode), `uptime_ms > 0`, `workers.len() == 3`, `fleet.len() == 1`, `last_event_seq` is a valid u64.

2. **`api_state_rejects_missing_bearer`** — GET `/api/state` no Authorization → 401.

3. **`ws_v2_replays_events_within_range`** — start ephemeral server. Pre-load `state.replay_buffer` with 5 AgentStreamEvent::ToolCall events with seq 1..=5 (use `state.replay_buffer.push(event)`). Connect via tokio_tungstenite. Send `{"action":"subscribe","last_seq":0}`. Assert client receives the `ReplayResponse{replay:"ok"}` ack, then 5 events in order with seqs 1..=5. **A stub that returns 0 events fails this.**

4. **`ws_v2_returns_out_of_range_when_client_too_far_behind`** — pre-load buffer with seq 1001..=1100 (push 100 events, last seqs ~1001-1100). Connect, send `{"action":"subscribe","last_seq":200}`. Assert client receives a single message that parses as `ReplayResponse{replay:"out_of_range",oldest_seq:Some(1001)}`. Then assert connection is closed (client's next recv returns None or close frame). **A stub that returns events anyway fails this.**

5. **`ws_v2_at_boundary_replays_correctly`** — pre-load with seq 50..=60. Connect with `last_seq:50`. Assert client receives ack + events with seq 51..=60 (ten events, NOT the seq=50 itself).

6. **`ws_v2_live_tail_after_historical_replay`** — pre-load with seq 1..=3. Connect, send `{"action":"subscribe","last_seq":0}`. Drain ack + 3 historical events. THEN publish a NEW event with seq=4 via `state.ws_events.send(envelope)` (wrap it via `daemon_core::encode_ws_event("agent_stream", ...)`). Wait briefly. Assert client receives the seq=4 event from the live tail. **The buffer-fill task from T-007.5 should also push it into replay_buffer in the background — that's fine.**

7. **`ws_v2_dedups_overlap_between_historical_and_live`** — pre-load buffer with seq 1..=5. Connect with `last_seq:0`. Drain ack + the 5 historical events. After draining, publish a duplicate envelope for seq=3 via ws_events.send. Wait briefly. Assert client does NOT receive a second seq=3 message (max_sent dedup catches it). **A stub without dedup fails this.**

8. **`ws_v2_rejects_missing_bearer_on_upgrade`** — connect with no Authorization header. Assert the upgrade fails with HTTP 401 BEFORE the WebSocket switches protocols.

9. **`legacy_ws_route_unchanged`** — connect to GET `/ws` (NOT /ws/v2) — assert it still emits the 4 hardcoded bootstrap events from `daemon_http::ws_route` (`agent_state`, `fleet_progress`, `ledger_health`, `review_completed`). Backwards compat regression check.
